...
namespace VLB
{
    ...
    public class VolumetricLightBeamEditor : EditorCommon
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                EditorGUILayout.PropertyField(cameraClippingDistance, EditorStrings.Beam.CameraClippingDistance);

                var content = new GUIContent(EditorStrings.Beam.DepthBlendDistance);
        ...
        {
            foreach (var target in m_Targets) Undo.AddComponent<T>(target.gameObject);
        }
        ...
        {
            if (m_Targets.Count > 1 && GpuInstancing.isSupported)
            {
                string reasons = "";
                for (int i = 1; i < m_Targets.Count; ++i)
                {
                    if (!GpuInstancing.CanBeBatched(m_Targets[0], m_Targets[i], ref reasons))
                    {
